...
/** When false (default), Deepgram does not add sentence periods/commas; use spoken "point"/"dot" instead (see verbal-period.js). */
function deepgramAutoPunctuation() {
    return String(process.env.DEEPGRAM_PUNCTUATE || '').trim() === '1';
}
...
function getDeepgramLiveWsUrl() {
    ...
    const punct = deepgramAutoPunctuation();
    const qs = new URLSearchParams({
        model,
        language: lang,
        smart_format: punct ? 'true' : 'false',
        punctuate: punct ? 'true' : 'false',
        numerals: 'true',
    ...
    const parts = (keytermsRaw ? keytermsRaw : defaultKeyterms.join('|'))
        .split(/[|,]/)
        .map((s) => s.trim())
        .filter(Boolean)
        .slice(0, 40);
